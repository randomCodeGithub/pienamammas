...
?>
		<div id="fep-message-head-<?php echo fep_get_the_id(); ?>" class="fep-message-head<?php echo (isset($_GET['fep_id']) && absint($_GET['fep_id']) === fep_get_the_id()) ? ' fep-message-head-active' : ''; ?>" data-fep_id="<?php echo fep_get_the_id(); ?>">
			...
			<?php
				...
				if (
					apply_filters('fep_remove_own_avatar_from_messagebox', false)
			...
			?>
				<div class="fep-avatar-p <?php echo (count($participants) > 2) ? 'fep-avatar-p-120' : 'fep-avatar-p-90' ?>">
					...
					?>
						...
						<?php if (get_current_user_id() != $p) :
							$userMessagePhotoLink = '';
							if (get_user_meta($p, 'user-avatar', true) == '') {
								$userMessagePhotoLink = get_stylesheet_directory_uri() . '/assets/img/profile/profile-default.svg';
							} else {
								$userMessagePhotoLink = apply_filters('um_user_shortcode_filter__profile_photo', get_user_meta($p, 'user-avatar', true), $p);
							}
						?>
							<!-- <img src="<?php echo $userMessagePhotoLink ?>" style="width: 50px; background-color: grey" alt=""> -->
							<div class="img-block" style="background-image: url(<?php echo $userMessagePhotoLink ?>); background-size: cover;background-position: center;
							<?php if (get_user_meta($p, 'user-avatar', true) == '') echo 'background-size: contain;'; ?>"></div>
						<?php endif; ?>
